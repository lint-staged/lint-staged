...
const MIN_GIT_VERSION = [2, 27, 0]
const parseGitVersion = (s) => {
  const m = s.match(/git version\s+(\d+)\.(\d+)\.(\d+)/i)
  return m ? [Number(m[1]), Number(m[2]), Number(m[3])] : null
}
const isAtLeast = (v, min) => {
...
const lintStaged = async (
  ...
  const parsedGitVersion = parseGitVersion(gitVersion)
  if (!parsedGitVersion || !isAtLeast(parsedGitVersion, MIN_GIT_VERSION)) {
    const detected = parsedGitVersion ? parsedGitVersion.join('.') : gitVersion.trim()
    logger.error(MIN_GIT_VERSION_REQUIRED(MIN_GIT_VERSION.join('.'), detected))
    return false
  }
