...
const getTag = ({ code, isTerminated, signal }) =>
  (isTerminated && 'KILLED') || signal || code || 'FAILED'
 ...
 * @param {boolean} result.isTerminated
...
const killExecaProcess = async (execaProcess) => {
  ...
  // The execa process is terminated separately in order to get the `KILLED` status.
  execaProcess.kill()
}
...
export const resolveTaskFn = ({
  ...
  return async (ctx = getInitialState()) => {
    ...
    if (result.failed || result.isTerminated || result.signal != null) {
