 ...
 * @returns {Error}
 */
const makeErr = (cmd, { all, code, killed, signal }, context = {}) => {
  ...
  let message = `${symbols.error} ${chalk.redBright(
    `${task} found some errors. Please fix them and try committing again.`
  )}`
  if (all) message += '\n' + all // append task output. `all` includes bot stdout and stderr.
  return createError(message)
}
...
const resolveTaskFn = ({ command, files, gitDir, isFn, relative, shell = false }) => {
  ...
  const execaOptions = {
    all: true, // `all` includes bot stdout and stderr.
    preferLocal: true,
    reject: false,
    shell
  }
