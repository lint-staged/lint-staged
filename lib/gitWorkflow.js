...
/**
 * @type {LintStaged.Adapter}
 */
class GitWorkflow {
  /**
   * TODO: will be done, if interface is ok for you
   * @param {Object} options
   * @param {boolean} options.allowEmpty
   * @param {boolean} options.stash
   * @param {boolean} options.cwd
   * @param {Object} logger
   */
  constructor({ allowEmpty, stash, cwd }, logger = console) {
    ...
    this.deletedFiles = []
    this.gitDir = null
    this.gitConfigDir = null
    this.unstagedDiff = null
  }

  async init(ctx) {
    const { gitDir, gitConfigDir } = await resolveGitRepo(this.cwd)
    if (!gitDir) {
      if (!ctx.quiet) ctx.output.push(NOT_GIT_REPO)
      ctx.errors.add(GitRepoError)

      throw Object.assign(new Error('lint-staged failed'), { ctx })
    }
    ...
    const shouldBackup = this.stash && hasInitialCommit

    if (!shouldBackup) {
      this.logger.warn(skippingBackup(hasInitialCommit))
    }

    return { baseDir: gitDir, shouldBackup }
  }

  async getStagedFiles() {
    return getStagedFiles({ cwd: this.gitDir })
  }

  executeTasksSkipped(ctx) {
  ...
  async applyModifications(ctx) {
    ...
    for (const files of ctx.matchedFileChunks) {
  ...
  hasPartiallyStagedFiles(ctx) {
    return ctx.hasPartiallyStagedFiles
  }

  applyModificationsSkipped(ctx) {
  ...
  restoreOriginalStateEnabled(ctx) {
    return (
      ctx.shouldBackup &&
      (ctx.failedTasks ||
  ...
  restoreOriginalStateSkipped(ctx) {
  ...
  restoreUnstagedChangesSkipped(ctx) {
    // Should be skipped in case of git errors
    if (ctx.errors.has(GitError)) {
      return GIT_ERROR
    }
