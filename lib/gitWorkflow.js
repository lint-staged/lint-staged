  ...
  async updateIndex(ctx) {
    ...
    try {
      ...
      if (ctx.shouldFailOnChanges) {
        ...
        const unstagedDiffSha256 = calculateSha256(ctx.unstagedDiff)
        const diffSha256 = calculateSha256(diff)
        debugLog('SHA-256 hash of unstaged changes is %s', unstagedDiffSha256)
        debugLog('SHA-256 hash of changes after tasks is %s', diffSha256)

        if (unstagedDiffSha256 !== diffSha256) {
