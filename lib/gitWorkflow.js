...
class GitWorkflow {
  ...
  async applyModifications(ctx) {
    ...
    for (const files of matchedFileChunks) {
      let filesMapped = files
      const cwd = process.cwd()
      if (this.gitDir && cwd.includes(this.gitDir) && cwd !== this.gitDir) {
        const append = cwd.replace(this.gitDir + '/', '')
        filesMapped = files.map((file) => {
          return append + '/' + file
        })
      }
      await this.execGit(['add', '--', ...filesMapped])
    }
