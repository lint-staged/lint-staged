    ...
    try {
      ...
      if (ctx.shouldBackup) {
        ...
        if (ctx.shouldHideUnstaged || ctx.shouldHideAll) {
          ...
          const objectId = await this.execGit(['stash', 'create'])
          ctx.backupHash = await this.execGit(['rev-parse', '--short', objectId])

          await this.execGit(['stash', 'store', '--quiet', '--message', stashMessage, objectId])
        }
