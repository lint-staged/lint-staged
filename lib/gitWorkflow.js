...
export class GitWorkflow {
  ...
  constructor({
    ...
    /** 8 characters, similar to a git short hash */
    this.runId = crypto.randomBytes(4).toString('hex')
    debugLog('Git workflow with run id %s', this.runId)
  }
  ...
  async prepare(ctx) {
    ...
    try {
      ...
      if (ctx.shouldBackup) {
        const stashMessage = `lint-staged automatic backup (${this.runId})`
        ...
        if (ctx.shouldHideUnstaged || ctx.shouldHideAll) {
          ...
          ctx.backupHash = stashes.find((line) => line.includes(this.runId))?.split(' ')[0]
        ...
        ctx.backupHash = await this.execGit(['rev-parse', '--short', ctx.backupHash])
      }
