...
import { mkdirSync, readFile, rename, rm, unlink, writeFile } from './file.js'
...
export class GitWorkflow {
  ...
  async prepare(ctx, task) {
    try {
      ...
      if (this.partiallyStagedFiles || this.unstagedFiles || this.untrackedFiles) {
        ...
        if (ctx.shouldHideUntracked && this.untrackedFiles) {
  ...
  async hideUntrackedFiles(ctx) {
    try {
      for (const file of this.untrackedFiles) {
        // TODO: files are moved rather than deleted because they're not stored in the backup stash
        await rename(path.resolve(this.topLevelDir, file), this.getHiddenFilepath(file))
      }
    } catch (error) {
      handleError(error, ctx, HideUntrackedFilesError)
    }
  }
  ...
  async restoreOriginalState(ctx) {
    try {
      ...
      await rm(this.getHiddenDirectory())
