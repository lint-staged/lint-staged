...
export class GitWorkflow {
  ...
  async applyModifications(ctx) {
    if (this.add) {
      ...
      debugLog('Skipping automatic adding due to --no-add flag')
    }
    ...
    // If adding is disabled we intentionally don't error here: tasks modify working
    // directory but we did not add those changes to the index.
    if (!stagedFilesAfterAdd && !this.allowEmpty && this.add) {
