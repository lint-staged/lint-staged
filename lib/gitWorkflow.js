...
import { ensureDir, readFile, removeDir, writeFile } from './file.js'
...
const PATCH_UNSTAGED = 'unstaged.patch'
...
const DIFF_GIT_REGEXP = /^diff --git /m

/** Parse `git diff -z --raw --patch` output to map of filename --> diff */
const parseGitDiffRawWithPatch = (diff) => {
  if (!diff) return new Map()
  const [raw, patch] = diff.split('\u0000\u0000')
  const files = parseGitZOutput(raw).filter((_, index) => index % 2 === 1)
  const patches = patch.split(DIFF_GIT_REGEXP).slice(1)
  const filenames = files.values()
  const result = new Map()
  let previousHeader
  let filename

  for (const filePatch of patches) {
    const header = filePatch.slice(0, filePatch.indexOf('\n'))

    // With --no-renames, type changes produce consecutive blocks with identical headers.
    // Advance the raw filename only when starting a new header group.
    if (header !== previousHeader) {
      filename = filenames.next().value
      previousHeader = header
    }

    result.set(filename, (result.get(filename) ?? '') + filePatch)
  }
...
export class GitWorkflow {
  ...
  async getUnstagedFiles({ onlyPartial = false } = {}) {
    ...
    const status = await this.execGit(['status', '-z', '--untracked-files=no'])
    /**
    ...
    const unstagedFiles = status
      ...
      .filter((line) => {
        ...
        const updatedInIndex = index !== ' '
        const updatedInWorkingTree = workingTree !== ' '
  ...
  /** Print stash list with commit hash and subject */
  async listStashes() {
    return await this.execGit(['stash', 'list', '--format="%H %s"', '-z'])
  ...
  async prepare(ctx) {
    ...
    try {
      this.hiddenPath = path.join(this.gitConfigDir, `./lint-staged/${ctx.runId}`)
      ...
      if (this.unstagedFiles) {
        await ensureDir(this.hiddenPath)
        ctx.unstagedPatch = path.join(this.hiddenPath, PATCH_UNSTAGED)
      ...
      if (ctx.shouldBackup) {
        const stashMessage = `lint-staged automatic backup (${ctx.runId})`

        if (ctx.shouldHideUnstaged || ctx.shouldHideAll) {
          const args = ['stash', 'push', '--keep-index', '--message', stashMessage]
          ...
          /** The stash line starts with the commit hash, so we split from space and choose the first part */
          ctx.backupHash = stashes.find((line) => line.includes(ctx.runId))?.split(' ')[0]

          await this.restoreMergeStatus(ctx)
        } else {
          ctx.backupHash = await this.execGit(['stash', 'create'])
        ...
        debugLog('Created backup stash with commit hash %s', ctx.backupHash)
      }

      this.logger.log(
        `${figures.done()} ${
          ctx.shouldBackup
            ? `Done backing up original state (${await this.execGit(['rev-parse', '--short', ctx.backupHash])})!`
  ...
  async runTasks(ctx, tasks, { abortController, concurrent }) {
    ...
    try {
      ctx.unstagedDiff = await this.execGit(
        ['diff', ...GIT_DIFF_ARGS, '--raw', '-z', '--no-renames', '--no-textconv'],
        { trim: false }
      )
    } catch (error) {
      ctx.errors.add(GitError)
      this.logger.error('Failed to get diff of unstaged changes!')
      debugLog(error)
      return
    }
  ...
  async updateIndex(ctx) {
    ...
    try {
      ...
      const before = parseGitDiffRawWithPatch(ctx.unstagedDiff)
      const after = parseGitDiffRawWithPatch(diff)

      /** @type {Set<string>} */
      const addableFiles = new Set()
      for (const file of new Set([...before.keys(), ...after.keys()])) {
        if (before.get(file) !== after.get(file)) {
          addableFiles.add(normalizePath(path.resolve(this.topLevelDir, file)))
        }
      }

      if (ctx.shouldFailOnChanges && addableFiles.size > 0) {
        ctx.errors.add(FailOnChangesError)
        this.logger.error(`${figures.error()} Tasks modified files and --fail-on-changes was used!`)
        return
      }

      if (addableFiles.size > 0) {
        const addCommands = await chunkFilesForCommand(
          'git add --',
          [...addableFiles],
          this.maxArgLength
        )
        ...
        const defaultIndexLock = activeIndexFile?.endsWith('.lock')
          ? normalizePath(
              await this.execGit([
                'rev-parse',
        ...
        for (const { files } of addCommands) {
           ...
           */
          if (defaultIndexLock && activeIndexFile !== defaultIndexLock) {
      ...
      if (!this.allowEmpty) {
        const stagedFilesAfterAdd = parseGitZOutput(
          await this.execGit([...getDiffCommand(this.diff, this.diffFilter), '--name-only', '-z'])
        )

        if (stagedFilesAfterAdd.length === 0) {
  ...
  async restoreUnstagedChanges(ctx) {
    ...
    const unstagedPatch = path.join(this.hiddenPath, PATCH_UNSTAGED)

    try {
      await this.execGit(['apply', ...GIT_APPLY_ARGS, unstagedPatch])
      await removeDir(this.hiddenPath)
    } catch (applyError) {
  ...
  async restoreUntrackedFiles(ctx) {
    ...
    try {
      const untrackedTree = `${ctx.backupHash}^3`
  ...
  async restoreOriginalState(ctx) {
    ...
    try {
      ...
      await this.execGit(['stash', 'apply', '--quiet', '--index', ctx.backupHash])
      await this.restoreMergeStatus(ctx)
      await removeDir(this.hiddenPath)
  ...
  async cleanup(ctx) {
    ...
    try {
      ...
      if (index > -1) {
        await this.execGit(['stash', 'drop', '--quiet', String(index)])
