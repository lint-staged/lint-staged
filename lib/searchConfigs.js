...
const CONFIG_PATHSPEC = CONFIG_FILE_NAMES.map((f) => `*${f}`)
...
export const searchConfigs = async (
  ...
  const [cachedFilesWithStatus, otherFilesWithStatus] = await Promise.all([
    ...
    execGit([...EXEC_GIT, '--', ...CONFIG_PATHSPEC], { cwd: topLevelDir }).then(parseGitZOutput),
  ...
  const possibleConfigFiles = [...cachedFilesWithStatus, ...otherFilesWithStatus]
    .flatMap((line) => {
      ...
      if (isInsideDirectory(cwd, absolutePath)) {
        return [absolutePath]
      }
