...
export const searchConfigs = async (
  ...
  const [fromGit, fromFs] = await Promise.all([
    listConfigFilesFromGit({ cwd, topLevelDir }),
    listConfigFilesFromFs({ cwd }),
  ])

  possibleConfigFiles.push(...fromGit)
  possibleConfigFiles.push(...fromFs)
