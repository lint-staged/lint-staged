...
export const searchConfigs = async (gitDir = process.cwd(), logger) => {
  ...
  /** Add possible root config file, unless it was already found */
  const absoluteFiles = [...cachedFiles, ...otherFiles].map((file) => join(gitDir, file))
  if (rootConfigPath && !absoluteFiles.includes(rootConfigPath)) {
    absoluteFiles.push(rootConfigPath)
  }

  /** Sort possible config files so that deepest is first */
  const possibleConfigFiles = absoluteFiles.map((file) => normalize(file)).sort(sortDeepestParth)
