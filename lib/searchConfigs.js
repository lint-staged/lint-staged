...
const compareConfigPaths = (a, b) => numberOfLevels(b) - numberOfLevels(a) || a.localeCompare(b)

const GIT_STATUS_REGEXP = /^[HSMRCK?U] /
  ...
  execGit(
    ...
    .then((lines) => {
      const possibleConfigFiles = lines.flatMap((line) => {
        ...
        const relativePath = line.replace(GIT_STATUS_REGEXP, '')
 ...
 *
 * @returns {Promise<Record<string, import('./config.js').Configuration>>} found configs with filepath as key, and config as value
 */
export const searchConfigs = async (
  ...
  const configs = [...possibleConfigFiles]
    .toSorted(compareConfigPaths)
    .reduce((acc, groupConfigPath) => Object.assign(acc, { [groupConfigPath]: null }), {})

  /** Load and validate all configs to the above object */
  await Promise.all(
    Object.keys(configs).map((groupConfigPath) =>
      loadConfig(groupConfigPath, logger).then(({ config, filepath }) => {
        if (config) {
          if (groupConfigPath !== filepath) {
            debugLog('Config file "%s" resolved to "%s"', groupConfigPath, filepath)
          }

          configs[groupConfigPath] = validateConfig(config, filepath, logger)
        }
      })
    )
  )
