...
/**
 * Validate commands are sequential:
 * - a single string/task
 * - an array of strings/tasks, running sequentially, with at most one level of nested arrays
 *   for running inner strings/tasks in parallel
 */
export const isSequentialTasks = (commands, depth = 0, allowFunctions = true) => {
  if (Array.isArray(commands)) {
    return (
      depth < 2 &&
      commands.every((command) => isSequentialTasks(command, depth + 1, allowFunctions))
    )
  }

  return typeof commands === 'string' || (allowFunctions && typeof commands === 'function')
}
...
export const validateConfigLogic = (config, configPath, logger) => {
  ...
  debugLog('Validated config from `%s`:\n%O', configPath, config)
