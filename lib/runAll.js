...
import { getAddedFilesWithoutIta, getStagedFiles } from './getStagedFiles.js'
...
export const runAll = async (
  ...
  if (!ctx.shouldBackup) {
    thisLogger.warn(skippingBackup(all, hasInitialCommit, diff))
  }
  ...
  const [stagedFiles, foundConfigs] = await Promise.all([
    getStagedFiles({ all, cwd: topLevelDir, diff, diffFilter, gitConfigDir }),
  ...
  /** If there are added files, re-check without `--intent-to-add` because Git stash doesn't support them */
  if (stagedFiles.some((f) => f.status === 'A')) {
    const addedFiles = stagedFiles.flatMap((f) => (f.status === 'A' ? f.filepath : []))
    const addedWithoutIta = await getAddedFilesWithoutIta({ cwd: topLevelDir, diff, diffFilter })
    const itaFiles = addedFiles.filter((f) => !addedWithoutIta.includes(f))
    if (itaFiles.length > 0) {
      ctx.errors.add(IntentToAddError)
      throw createError(ctx, itaFiles)
    }
  }
