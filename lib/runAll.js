 ...
 * @param {Array} [options.files] - List of manual files for checks
 * @param {Logger} logger
 * @returns {Promise}
 */
export const runAll = async (
  ...
  // TODO, make within commander.js
  if (userFiles) {
    stash = false
  }

  // TODO, skip git exec, if set options --files
  ...
  const files = userFiles || (await getStagedFiles({ cwd: gitDir, diff, diffFilter }))
  if (!files) {
