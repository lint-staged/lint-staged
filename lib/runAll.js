...
import { getAllFiles, getStagedFiles } from './getStagedFiles.js'
 ...
 * @param {boolean} [options.all] - Run on all tracked files instead of just staged files
...
export const runAll = async (
  ...
  const [stagedFiles, foundConfigs] = await Promise.all([
    // Use getAllFiles when --all flag is set, otherwise use getStagedFiles
    all
      ? getAllFiles({ cwd: topLevelDir })
