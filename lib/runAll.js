...
const { DEPRECATED_GIT_ADD, FAILED_GET_STAGED_FILES, NO_TASKS, TASK_ERROR } = require('./messages')
...
const runAll = async (
  ...
  const adapter = createVcsAdapter(vcsAdapter, { allowEmpty, stash, cwd }, logger)

  const { baseDir, shouldBackup } = await adapter.init(ctx)
  ...
  for (const [index, files] of stagedFileChunks.entries()) {
    ...
    listrTasks.push({
      ...
      skip: () => {
        ...
        return adapter.executeTasksSkipped(ctx) || false
      },
    })
  }
  ...
  const runner = new Listr(
    [
      {
        title: 'Preparing...',
        task: (ctx) => adapter.prepare(ctx),
      },
      {
        title: 'Hiding unstaged changes to partially staged files...',
        task: (ctx) => adapter.hideUnstagedChanges(ctx),
        enabled: (ctx) => adapter.hasPartiallyStagedFiles(ctx),
      },
      ...listrTasks,
      {
        title: 'Applying modifications...',
        task: (ctx) => adapter.applyModifications(ctx),
        skip: (ctx) => tasksFailed(ctx) || adapter.applyModificationsSkipped(ctx),
      },
      {
        title: 'Restoring unstaged changes to partially staged files...',
        task: (ctx) => adapter.restoreUnstagedChanges(ctx),
        enabled: (ctx) => adapter.hasPartiallyStagedFiles(ctx),
        skip: (ctx) => tasksFailed(ctx) || adapter.restoreUnstagedChangesSkipped(ctx),
      },
      {
        title: 'Reverting to original state because of errors...',
        task: (ctx) => adapter.restoreOriginalState(ctx),
        enabled: (ctx) => adapter.restoreOriginalStateEnabled(ctx),
        skip: (ctx) => adapter.restoreOriginalStateSkipped(ctx),
      },
      {
        title: 'Cleaning up...',
        task: (ctx) => adapter.cleanup(ctx),
        enabled: (ctx) => adapter.cleanupEnabled(ctx),
        skip: (ctx) => adapter.cleanupSkipped(ctx),
      },
    ],
    listrOptions
  )
