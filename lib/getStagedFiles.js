 ...
 */
export const getStagedFiles = async ({ all, cwd, gitConfigDir, diff, diffFilter }) => {
  try {
    if (all) {
      const allFiles = await execGit(['ls-files', '--full-name', '-z'])
      return parseGitZOutput(allFiles).map((filename) => ({
        filepath: normalizePath(path.resolve(cwd, filename)),
        status: 'X',
      }))
    }
    ...
    const getFiles = async (command) => {
       ...
       */
      const output = await execGit([...command, '--ita-visible-in-index', '--raw', '-z'], { cwd })
...
export const getAddedFilesWithoutIta = async ({ cwd, diff, diffFilter }) => {
  const addedWithoutIta = await execGit(
    [...getDiffCommand(diff, diffFilter), '--ita-invisible-in-index', '--raw', '-z'],
    { cwd }
  )

  return parseGitZOutput(addedWithoutIta).map((filename) =>
