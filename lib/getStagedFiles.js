 ...
 * @returns {Promise<StagedFile[] | null>}
 */
export const getAllFiles = async ({ cwd = process.cwd() } = {}) => {
  try {
    const output = await execGit(['ls-files'], { cwd })
    ...
    return output
      ...
      .map((filename) => ({
        ...
        // Use 'M' status for all files since there's no actual diff
        // This is arbitrary but consistent with "modified" files in diff mode
        status: 'M',
      }))
  } catch {
    return null
