...
export const execGit = async (cmd, options) => {
  ...
  let encounteredGitCrlfWarning = false
  let output = ''
  for await (const line of result) {
    if (!encounteredGitCrlfWarning && GIT_CRLF_WARNING.test(line)) {
      encounteredGitCrlfWarning = true
      debugLog('Stripped Git CRLF warning:\n%s', line)
      continue
    }
