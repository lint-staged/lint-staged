...
export const loadConfig = async ({ configPath, cwd }, logger) => {
  try {
    ...
    if (config.extends) {
      const sharedConfigResult = explorer.load(resolveConfig(config.extends))

      if (!sharedConfigResult) {
        throw new Error('Failed to load shared config from `%s`', config.extends)
      }

      delete config.extends
      const sharedConfig = await sharedConfigResult
      const extendedConfig = { ...sharedConfig.config, ...config }

      return { config: extendedConfig, filepath }
    }
