...
import GitWorkflow from '../lib/__mocks__/gitWorkflow'
...
jest.mock('../lib/createVcsAdapter')
...
createVcsAdapter.mockImplementation((...args) => {
  return new GitWorkflow(args)
})
...
describe('runAll', () => {
  ...
  it('should exit with error if getting staged files failed', async () => {
    expect.assertions(1)
    getStagedFiles.mockImplementationOnce(async () => undefined)
    await expect(
      runAll({ config: { '*.css': ['echo "sample"'] }, quiet: true })
  ...
  it('should skip tasks if previous git error', async () => {
    expect.assertions(2)
    getStagedFiles.mockImplementationOnce(async () => ['sample.js'])
    createVcsAdapter.mockImplementationOnce((...args) => {
      const gitWorkflow = jest.requireActual('../lib/__mocks__/gitWorkflow')
      class gitWorkflowOverrided extends gitWorkflow {
        prepare(ctx) {
      ...
      return new gitWorkflowOverrided(args)
    })
  ...
  it('should resolve matched files to cwd when using relative option', async () => {
    ...
    expect(ctx.matchedFileChunks).toEqual([[normalize(path.join(cwd, 'test/foo.js'))]])
  })
})
